...
rookieDB 是本项目的外部依赖（`/Users/shunhewang/melbourne/cs186/berkeley-sp26-rookiedb`）。**任何对 rookieDB 源码或测试的修改，必须先读 `.claude/skills/rookiedb-safety/SKILL.md`**。这个 skill 记录了 4 个 rookieDB 特有的设计约束（Phaser 生命周期、ThreadLocal 事务模型、LockManager.acquire() 静默失败、Transaction 状态机隐藏分支）和按文件分类的修改 checklist。DDA 测试中踩出的 5 个 bug 全是因为违反这些约束——不看就动手，几乎必然出问题。
